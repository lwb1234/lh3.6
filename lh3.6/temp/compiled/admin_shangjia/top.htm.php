...
.authorize-icon {
    background: url("./images/authorize.png") no-repeat scroll 20px 0 rgba(0, 0, 0, 0);
...
.noauthorize-icon a {
    background: url("./images/noauthorize.png") no-repeat scroll 20px 0 rgba(0, 0, 0, 0);
    border-left: 1px solid #999999;
    display: inline-block;
    height: 24px;
    line-height: 24px;
...
function start_sendmail_Response(result)
{
  ...
  if (result.error == 0)
  {
	...
	if (result.error == 0)
	{
		...
		{
             str =' <span class="authorize-icon"> <a  target="_blank" href="'+result['content']['auth_str']+'"></a></span>';
		}
		else
		{
            str =' <span class="noauthorize-icon"> <a target="main-frame" href="'+result['content']['auth_str']+'"></a></span>';
		}
        if(result['content']['auth'])
		{
		    str += ' <span > <a  target="_blank" href="'+result['content']['auth_str']+'" >' + result['content']['auth_type'];
		}
		else
		{
		    str += ' <span > <a  target="main-frame" href="'+result['content']['auth_str']+'" >' + result['content']['auth_type'];
		}
		str += '</a> </span>';
...
<body>
<div id="header-div">
  <div id="logo-div" style="bgcolor:#000000;"><img src="images/lihua_logo.gif" alt="黎华家具城" /></div>
  <div id="submenu-div">
    <ul>
      ...
      <li><a href="../seller_store.php?sid=<?php echo $this->_var['store_id']; ?>" target="_blank">查看我的店铺</a></li>
