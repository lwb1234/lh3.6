...
function get_seller_top10($seller_id=0,$cats = '',$top_time='',$top_number=10)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name,g.market_price, g.promote_price, g.shop_price, g.goods_thumb, SUM(og.goods_number) as goods_number ' .
        ...
        "WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 and g.is_display=1 and g.seller_id='$seller_id' AND g.is_delete = 0 $where $top10_time " ;
    //判断是否启用库存，库存数量是否大于0
 ...
 * @return  array
 */
function store_goods($type, $brand, $cat = 0)
{
    static $result = NULL;
 ...
 * by lee
 * 取得秒杀活动信息
 * @param   int     $seckill_buy_id   秒杀活动id   修改成商品参加秒杀的商品id
 * @param   int     $current_num    本次购买数量（计算当前价时要加上的数量）
 * @return  array
 */
function seckill_buy_info($seckill_buy_id = 0, $current_num = 0)
{
    /* 取得秒杀活动信息  在活动表中加了一个字段seckill_price */
    $seckill_buy_id = intval($seckill_buy_id);
    $sql = "SELECT *, act_id AS seckill_buy_id,act_desc AS seckill_buy_desc, seckill_price,start_time AS start_date, end_time AS end_date " .
        //"FROM " . $GLOBALS['ecs']->table('goods_activity') .
        "FROM " . $GLOBALS['ecs']->table('goods_activity') . " AS b " .
        "LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON b.goods_id = g.goods_id " .
//        "WHERE act_id = '$seckill_buy_id' " .   这里做了修改，将商品的分类ID 改为参加秒杀活动商品ID
        "WHERE g.goods_id = '$seckill_buy_id' " .
        "AND act_type = '" . GAT_SECKILL . "'";

    $seckill_buy = $GLOBALS['db']->getRow($sql);
    /* 如果为空，返回空数组 */
    ...
    /* 格式化时间 */
//    时间的字段没有处理，所以使用原来的字段的
    $seckill_buy['formated_start_date'] = local_date('Y-m-d H:i', $seckill_buy['start_date']);
    $seckill_buy['formated_end_date'] = local_date('Y-m-d H:i', $seckill_buy['end_date']);

    /* 格式化保证金 */
    $seckill_buy['formated_deposit'] = price_format($seckill_buy['deposit'], false);

    /* 处理价格阶梯 */
    $price_ladder = $seckill_buy['price_ladder'];   //空值 ，价格阶梯
    ...
    /* 统计信息 */
    $stat = seckill_buy_stat($seckill_buy_id, $seckill_buy['deposit']);

    $seckill_buy = array_merge($seckill_buy, $stat);
    /* 计算当前价 */
    ...
    $seckill_buy['cur_price'] = $cur_price;
    $seckill_buy['formated_cur_price'] = $cur_price;

    /* 最终价 */
    $seckill_buy['trans_price'] = $seckill_buy['cur_price'];
    $seckill_buy['formated_trans_price'] = $seckill_buy['formated_cur_price'];
    $seckill_buy['trans_amount'] = $seckill_buy['valid_goods'];

    $seckill_buy['formated_rebate_price'] = price_format($seckill_buy['market_price'] - $cur_price);
    $seckill_buy['t_discount'] = sprintf ("%01.1f", $cur_price/$seckill_buy['market_price']*10);
    $seckill_buy['market_price'] = price_format($seckill_buy['market_price']);

    /* 秒杀活动状态 */
    $seckill_buy['status'] = seckill_buy_status($seckill_buy);
    if (isset($GLOBALS['_LANG']['gbs'][$seckill_buy['status']]))
    {
        $seckill_buy['status_desc'] = $GLOBALS['_LANG']['gbs'][$seckill_buy['status']];
    }

    $seckill_buy['start_time'] = $seckill_buy['formated_start_date'];
    $seckill_buy['end_time'] = $seckill_buy['formated_end_date'];
    $seckill_buy['isg_new'] = $seckill_buy['isg_new'];
...
/*
 * 取得某秒杀活动的统计信息
 * @param   int     $seckill_buy_id  秒杀活动id
 * @param   float   $deposit        保证金
 * @return  array   统计信息
 */
function seckill_buy_stat($seckill_buy_id, $deposit)
{
    $seckill_buy_id = intval($seckill_buy_id);
    ...
    $sql = "SELECT goods_id " .
        "FROM " . $GLOBALS['ecs']->table('goods_activity') .
//        "WHERE act_id = '$seckill_buy_id' " .
        //将这里的商品分类ID改成商品ID
        "WHERE goods_id = '$seckill_buy_id' " .
        "AND act_type = '" . GAT_SECKILL . "'";
    $seckill_buy_goods_id = $GLOBALS['db']->getOne($sql);
...
//        "AND o.extension_code = 'group_buy' " .
        ...
        "AND g.goods_id = '$seckill_buy_goods_id ' " .
 ...
 * @return  integer
 */
function seckill_buy_status($group_buy)
{
    $now = gmtime();
    if ($group_buy['is_finished'] == 0)
    {
    ...
    elseif ($group_buy['is_finished'] == SBS_SUCCEED)
    {
        /* 已处理，秒杀成功 */
        $status = SBS_SUCCEED;
    }
    elseif ($group_buy['is_finished'] == SBS_FAIL)
    {
        /* 已处理，秒杀失败 */
        $status = SBS_FAIL;
    }
