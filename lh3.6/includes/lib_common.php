 ...
 * @return  mix
 */
function cat_store_goods_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    static $res = NULL;


    $sql = "select g.goods_name ,s.shop_name ,g.goods_id,s.id, g.seller_cat_id,g.goods_number ,COUNT(s.seller_id) as goods_num ".
        "from" . $GLOBALS['ecs']->table('goods') . " AS g ,".
        $GLOBALS['ecs']->table('seller_shopinfo') . " AS s where g.seller_cat_id =s.seller_id".
        " GROUP BY s.id , g.goods_id ";
    ...
    $sql2 = "SELECT g.cat_id, COUNT(*) AS sellers_num " .
        " FROM " . $GLOBALS['ecs']->table('goods') . " AS g , " . $GLOBALS['ecs']->table('seller_shopinfo') . " AS s " .
        " WHERE g.goods_id = s.seller_id " ;
    " GROUP BY g.cat_id";
    $res3 = $GLOBALS['db']->getAll($sql2);



// @ 过滤   自定义的临时数组用来缓存seller_id 数值  by  lee 20170910  start
    $newres = '';
    global $temp ;
    foreach($res as $k=>$v) {
        if ( $temp == $v['seller_cat_id'] ) {
            //如果与上次遍历到的一样则将商户名称字段至空
            $v['shop_name'] = '';
            //将商户的seller_cat_id 也至空
//            $v['seller_cat_id'] = '';  由于后期需要将商户ID显示所以不用至空
            $newres [$k] ['goods_name']= $v['goods_name'];
            $newres [$k] ['shop_name']= $v['shop_name'];
            $newres [$k] ['id']= $v['id'];
            $newres [$k] ['goods_id']= $v['goods_id'];
            $newres [$k] ['seller_cat_id']= $v['seller_cat_id'];
            $newres [$k] ['haschild']= $v['haschild'];
            $newres [$k] ['goods_number']= $v['goods_number'];
        } else {
            //将新的商户ID 放到$temp 里
            $temp = $v['seller_cat_id'];
            //与上次的分类ID不一样继续遍历
            //放到新的数组里面
            $newres [$k] ['goods_name']= $v['goods_name'];
            $newres [$k] ['shop_name']= $v['shop_name'];
...
//<! --     自定义的临时数组用来缓存seller_id 数值  by  lee 20170910  end -- >
 ...
 * @return  mix
 */
function cat_listH($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    static $res = NULL;
    ...
    {
        //读取缓存文件
        $data = read_static_cache('cat_pid_releateH');
        //var_dump($data);exit;
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.parent_id,  c.sort_order, COUNT(s.cat_id) AS has_children ".
                'FROM ' . $GLOBALS['ecs']->table('categoryH') . " AS c ".
                "LEFT JOIN " . $GLOBALS['ecs']->table('categoryH') . " AS s ON s.parent_id=c.cat_id ".
                "GROUP BY c.cat_id ".
...
//                " WHERE is_delete = 0 AND is_on_sale = 1 " .
            ...
            $sql = "SELECT gc.cat_id, COUNT(*) AS goods_num " .
                " FROM " . $GLOBALS['ecs']->table('goodHs_cat') . " AS gc , " . $GLOBALS['ecs']->table('goodHs') . " AS g " .
//                " WHERE g.goodHs_id = gc.goodHs_id AND g.is_delete = 0 AND g.is_on_sale = 1 " .
                " WHERE g.goodHs_id = gc.goodHs_id AND g.is_delete = 0  " .
                " GROUP BY gc.cat_id";
            //
...
//            var_dump($res3);exit;
            ...
            if (count($res) <= 1000)
            {
                write_static_cache('cat_pid_releateH', $res);
            }
        }
        else
        {
    ...
    $options = cat_optionsH($cat_id, $res); // 获得指定分类下的子分类的数组
    ...
    else
    {
        foreach ($options AS $key => $value)
        {
            $options[$key]['url'] = build_uri('categoryH', array('cid' => $value['cat_id']), $value['cat_name']);
        }
 ...
 * @return  void
 */
function cat_optionsH($spec_cat_id, $arr)
{
    ...
    if (!isset($cat_options[0]))
    {
        ...
        //这里的配置文件是读取了同一个缓存文件所以是不行的下面要生成两个
        $data = read_static_cache('cat_option_staticH');
        if ($data === false)
        {
            while (!empty($arr))
            {
            ...
            {
                write_static_cache('cat_option_staticH', $options);
            }
        }
        else
        {
...
function get_promotion_info($goods_id = '')
{
    ...
    foreach ($res as $data)
    {
        switch ($data['act_type'])
        {
            ...
            //by lee 秒杀商品信息 start
            case GAT_SECKILL_BUY: //，，秒杀
                $group[$data['act_id']]['act_name'] = $data['act_name'];
                $group[$data['act_id']]['url'] = build_uri('seckill_buy', array('gbid' => $data['act_id']));
                $group[$data['act_id']]['time'] = sprintf($GLOBALS['_LANG']['promotion_time'], local_date('Y-m-d', $data['start_time']), local_date('Y-m-d', $data['end_time']));
                $group[$data['act_id']]['sort'] = $data['start_time'];
                $group[$data['act_id']]['type'] = 'seckill_buy';
                break;
            //by lee 秒杀商品信息 end
...
//
/**
 * 获得入驻商家指定20150718add分类下所有底层分类的ID
 *
 * @access  public
    ...
    switch ($app)
    {
       ...
       // end
        ...
        case 'group_buy':
            ...
            break;
        // by lee 20170923 增加秒杀商品链接  start
        case 'seckill_buy':
            if (empty($gbid))
            {
                return false;
            ...
            {
                $uri = $rewrite ? 'seckill_buy-' . $gbid : 'seckill_buy.php?act=view&amp;id=' . $gbid;
            }
            break;
        // by lee 20170923 增加秒杀商品链接  end
        case 'auction':
            if (empty($auid))
            {
                return false;
