...
//        $source=preg_replace("/([^a-zA-Z0-9_]{1,1})+(copy|fputs|fopen|file_put_contents|fwrite|eval|phpinfo)+( |\()/is", "", $source);
        ...
        //升级到了3.6 由于PHP版本是5.6.5 所以更改preg_replace
        $source=preg_replace_callback("/([^a-zA-Z0-9_]{1,1})+(copy|fputs|fopen|file_put_contents|fwrite|eval|phpinfo)+( |\()/is",  function ($matches) {
            return '';
        }, $source);
