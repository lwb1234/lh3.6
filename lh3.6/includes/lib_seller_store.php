...
{
	$sql="select * from ".$GLOBALS['ecs']->table('seller_shopslide')." where seller_id='$seller_id' order by img_order ASC";
...
function get_store_window($seller_id)
{
	...
	{
		if($win['win_type']&&!empty($win['win_goods']))
		{
...
function get_seller_cat_goods($cat_id)
{
    ...
    $sql = 'SELECT goods_id FROM ' . $GLOBALS['ecs']->table('goods') . " AS g WHERE g.seller_cat_id='$cat_id'";
