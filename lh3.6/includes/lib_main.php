...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    if ($filename != 'index')
    {
        //在数组里，增加一个分类松北 --category_songbei by lee start
        /* 处理有分类的 */
        if (in_array($filename, array('category','category_songbei', 'goods', 'article_cat', 'article', 'brand')))
        {
            /* 商品分类或商品 */
            if ('category' == $filename  ||'category_songbei'== $filename|| 'goods' == $filename || 'brand' == $filename)
            {
                if ($cat > 0)
                {
...
function assign_dynamic($tmp)
{
       ...
       foreach ($res AS $row)
    {
        switch ($row['type'])
        {
            ...
            break;
            // by lee 增加商店下的商品 start
//            case 4:
//                /* 商店的商品 */
//                $store_goods = store_goods($row['id'], $row['number']);
//
//                $GLOBALS['smarty']->assign('store_goods_' . $row['id'], $brand_goods['goods']);
//                $GLOBALS['smarty']->assign('goods_brand_' . $row['id'], $brand_goods['brand']);
//                break;
                // end
        }
 ...
 * @param   string  $link   by lee 增加一个秒杀自己的参数$hrefs= seckill_success 时候自动向下执行
 * @param   string  $href
