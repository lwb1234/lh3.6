...
elseif ($_REQUEST['step'] == 'get_cart_list')
{
    include_once('includes/cls_json.php');
	$result = array('error' => 0, 'message' => '', 'content' => '', 'goods_id' => '');
    $json  = new JSON;
...
//    var_dump($flow_type); 5
    ...
    /* 秒杀标志 */
    if ($flow_type == CART_SECKILL_BUY_GOODS)
    {
        $smarty->assign('is_seckill_buy', 5);
    }
    //end
...
elseif ($_REQUEST['step'] == 'shipping_change')
{
	/* 取得新配送列表 */
	include_once('includes/cls_json.php');
	 $json = new JSON;
    $result = array('content' => '');
	$consignee['country']=intval($_REQUEST['country']);

	$consignee['province']=intval($_REQUEST['province']);
	$consignee['city']=intval($_REQUEST['city']);
	$consignee['district']=intval($_REQUEST['district']);
    ...
    $smarty->assign('insure_disabled', $insure_disabled);
    $smarty->assign('cod_disabled',    $cod_disabled);
	$result['content']     = $smarty->fetch('library/flow_shipping_box.lbi');
...
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    /* 获得收货人信息 */
    $consignee = get_consignee($_SESSION['user_id']);
		$consignee['country']= $_REQUEST['country'] ?  $_REQUEST['country'] : 1;
		$consignee['province']= $_REQUEST['province'] ?  $_REQUEST['province'] :0;
		$consignee['city']= $_REQUEST['city'] ?  $_REQUEST['city'] :0;
		$consignee['district']= $_REQUEST['district'] ?  $_REQUEST['district'] :0;
    ...
    else
    {
        ...
        $result['content']     = $smarty->fetch('library/flow_order_total.lbi');
    }

    echo $json->encode($result);
    exit;
}
elseif ($_REQUEST['step'] == 'get_order_amount')
{
    ...
    $json = new JSON;
    $result = array('order_amount' => '0.00');
	
	if ($_SESSION['user_id'] && intval($_SESSION['user_id']) > 0)
	{
	...
	$result['formated_order_amount']     = $order['formated_order_amount'];
	$result['pay_online']     = $pay_online;
...
elseif ($_REQUEST['step'] == 'change_needinv')
{
    ...
    else
    {
        ...
        die($smarty->fetch('library/flow_order_total.lbi'));
    }
}
...
elseif ($_REQUEST['step'] == 'update_cart')
{
    ...
    //show_message($_LANG['update_cart_notice'], $_LANG['back_to_cart'], 'flows.php?step=checkout');
	ecs_header("Location: flow.php\n");//888
    exit;
}
    ...
    foreach ($arr AS $key => $val)
    {
        ...
        if (intval($GLOBALS['_CFG']['use_storage']) > 0 && $goods['extension_code'] != 'package_buy')
        {
            if ($row['goods_number'] < $val)
            {
                ...
                $row['goods_number'], $row['goods_number']),'返回上一页','flows.php?step=checkout');
                exit;
            }
            /* 是货品 */
            $goods['product_id'] = trim($goods['product_id']);
            if (!empty($goods['product_id']))
            {
                ...
                {
                    show_message(sprintf($GLOBALS['_LANG']['stock_insufficiency'], $row['goods_name'],
                    $product_number['product_number'], $product_number['product_number']), '返回上一页', 'flows.php?step=checkout');
                    exit;
                }
            }
        }
