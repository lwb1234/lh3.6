...
//	var_dump($keyword);
    ...
    if ($page > $max_page)
    {
      ...
      // by lee 临时显示shop_logo 替换了street_logo 增加了字段 shop_logo
	 $sql="select sh.id,sh.shop_name,sh.street_logo,sh.street_spjpg,sh.shop_title ,sh.shop_logo from ".$GLOBALS['ecs']->table('seller_shopinfo')." 
	 as sh where sh.status=1 ".$where;
	 ...
	 while ($row = $GLOBALS['db']->FetchRow($res))
	 {
		...
		//增加商店logo 暂时不显示
		$arr[$row['id']]['shop_logo']=str_replace('../','./',$row['shop_logo']);
//		var_dump($arr[$row['id']]['shop_logo']);exit;
