...
elseif ($_REQUEST['act'] == 'info')
{
	...
	$sql="select o.*,u.user_name from ".$GLOBALS['ecs']->table('commission')." AS o LEFT JOIN " .$GLOBALS['ecs']->table('admin_user'). " AS u ON u.seller_id=o.seller_id where id='$id'";
	$comm_info=$GLOBALS['db']->getRow($sql);
	
	//上一笔佣金
	$sql = "SELECT MAX(id) FROM " . $ecs->table('commission') . " as o WHERE id < '$comm_info[id]'";
	$smarty->assign('prev_id', $db->getOne($sql));
	//下一笔佣金
	$sql = "SELECT MIN(id) FROM " . $ecs->table('commission') . " as o WHERE id > '$comm_info[id]'";
	...
	{
		//统计总营业额
		$sql="select sum(goods_amount) as number from " . $GLOBALS['ecs']->table('order_info') ." where order_status IN ('1','5') AND shipping_status IN ('1','2') AND pay_status IN ('2','1') and seller_id='".$comm_info['seller_id']."' and add_time>=".$comm_info['prev_date']." and add_time<=".gmtime();
		$total_turnover=$GLOBALS['db']->getOne($sql);
		//获取商家分成百分比
		$sql="select fencheng from ".$GLOBALS['ecs']->table('user_seller')." where id='".$comm_info['seller_id']."'";
	...
	//获取入驻商家对应的会员id
	$sql="select user_id from ". $GLOBALS['ecs']->table('user_seller')." where id='".$comm_info['seller_id']."'";
...
elseif($_REQUEST['act'] == 'operate')
{
	...
	if($status>1)
	{
		$remark=strip_tags($_REQUEST['remark']);
		//跟新佣金列表记录
		$sql="update ".$GLOBALS['ecs']->table('commission')." set remark='$remark',operator='".
			 $_SESSION['admin_name']."' where id='$cid'";

		$update=$GLOBALS['db']->query($sql);
		if($update)
		{
			$links[0]['text']    = '返回佣金列表';
			$links[0]['href']    = 'seller_commission.php?act=list&' . list_link_postfix();
		
			sys_msg('备注信息更新成功', 0, $links);	
		}
	}
	else
	{
		$check_date=!empty($_REQUEST['check_date'])?local_strtotime($_REQUEST['check_date']):'';
		if($check_date>0&&$check_date>gmtime())
		{
			
			$total_turnover=floatval($_REQUEST['total_turnover']);
			$commission=floatval($_REQUEST['commission']);
			$check_mony=floatval($_REQUEST['check_mony']);
			$remark=strip_tags($_REQUEST['remark']);
			$seller_id=intval($_REQUEST['s_id']);
			//跟新佣金列表记录
			$sql="update ".$GLOBALS['ecs']->table('commission')." set curr_date='".gmtime()."',total_turnover='$total_turnover',commission='$commission',check_mony='$check_mony',status=2,check_date='$check_date',remark='$remark',operator='".
				 $_SESSION['admin_name']."' where id='$cid'";
			...
			{
				//插入数据库下次佣金的统计开始时间
				$sql="insert into ".$GLOBALS['ecs']->table('commission')." (seller_id,prev_date) values ('$seller_id',".gmtime().") ";
				...
				sys_msg('佣金信息已处理', 0, $links);	
			}
		}
		else
		{
			sys_msg('请选择您的转账时间,且时间不小于当前时间', 0);	
		}	
	}
 ...
 * @return void
 */
function comm_list($is_pager=true)
{
