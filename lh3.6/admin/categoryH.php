 ...
 * ECSHOP 商户分类管理程序
 * ============================================================================
 * * 版权所有 2005-2012 上海户派网络科技有限公司，并保留所有权利。
 * 网站地址: http://www.ecshop.com；
 * ----------------------------------------------------------------------------
 * 这不是一个自由软件！您只能在不用于户业目的的前提下对程序代码进行修改和
...
$exc = new exchange($ecs->table("categoryH"), $db, 'cat_id', 'cat_name');
...
{
    /* 获取分类列表 */
    $cat_listH = cat_listH(0, 0, false);
//  var_dump($cat_listH);exit;
    /* 模板赋值 */
    $smarty->assign('ur_here',      $_LANG['03_category_listH']);
    $smarty->assign('action_link',  array('href' => 'categoryH.php?act=add', 'text' => $_LANG['04_category_add']));
    $smarty->assign('full_page',    1);

    $smarty->assign('cat_info',     $cat_listH);

    /* 列表页面 */
    assign_query_info();
    $smarty->display('category_listH.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('category_listH.htm'));
}
/*------------------------------------------------------ */
//-- 添加商户分类
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('action_link',  array('href' => 'category.php?act=list', 'text' => $_LANG['03_category_listH']));

    $smarty->assign('goodHs_type_list',  goodHs_type_list(0)); // 取得商户类型
    $smarty->assign('attr_list',        get_attr_list()); // 取得商户属性

    $smarty->assign('cat_select',   cat_listH(0, 0, true));
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    if ($cat_info['filter_attr'])
    {
        ...
        foreach ($filter_attr AS $k => $v)
        {
            ...
            $filter_attr_list[$k]['goodHs_type_list'] = goodHs_type_list($attr_cat_id);  //取得每个属性的商户类型
    ...
    /* 模板赋值 */
    $smarty->assign('attr_list',        $attr_list); // 取得商户属性
    $smarty->assign('attr_cat_id',      $attr_cat_id);
    $smarty->assign('ur_here',     $_LANG['category_edit']);
    $smarty->assign('action_link', array('text' => $_LANG['03_category_listH'], 'href' => 'category.php?act=list'));
    ...
    $smarty->assign('cat_select',  cat_listH(0, $cat_info['parent_id'], true));
    $smarty->assign('goodHs_type_list',  goodHs_type_list(0)); // 取得商户类型
...
if ($_REQUEST['act'] == 'update')
{
    ...
    /* 判断上级目录是否合法 */
    $children = array_keys(cat_listH($cat_id, 0, false));     // 获得当前分类的所有下级分类
...
if ($_REQUEST['act'] == 'move')
{
    ...
    /* 模板赋值 */
    $smarty->assign('ur_here',     $_LANG['move_goodHs']);
    $smarty->assign('action_link', array('href' => 'category.php?act=list', 'text' => $_LANG['03_category_listH']));

    $smarty->assign('cat_select', cat_listH(0, $cat_id, true));
...
if ($_REQUEST['act'] == 'move_cat')
{
    ...
    /* 更新商户分类 */
    $sql = "UPDATE " .$ecs->table('goodHs'). " SET cat_id = '$target_cat_id' ".
           "WHERE cat_id = '$cat_id'";
    if ($db->query($sql))
    {
        /* 清除缓存 */
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    /* 当前分类下是否存在商户 */
    $goodHs_count = $db->getOne('SELECT COUNT(*) FROM ' .$ecs->table('goodHs'). " WHERE cat_id='$cat_id'");

    /* 如果不存在下级子分类和商户，则删除之 */
    if ($cat_count == 0 && $goodHs_count == 0)
    {
        /* 删除分类 */
...
function get_attr_list()
{
    $sql = "SELECT a.attr_id, a.cat_id, a.attr_name ".
           ...
           $GLOBALS['ecs']->table('goodHs_type') . " AS c ".
