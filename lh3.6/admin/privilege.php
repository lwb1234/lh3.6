    ...
    if($_REQUEST['type']=='yunqi'){
        ...
        if(isset($_GET['code']) && $_GET['code']){
            ...
            if($res['token'] and $res['params']){
                ...
                // 去掉新版本的passport_uid 护照ID  字段  by lee  start
...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    if(!empty($ec_salt))
    {
        // 去掉这个新版本的字段passport_id 云起账号  start
         /* 检查密码是否正确 */
         $sql = "SELECT user_id, user_name, password, last_login, action_list, last_login,suppliers_id,ec_salt".
            " FROM " . $ecs->table('admin_user') .
            " WHERE user_name = '" . $_POST['username']. "' AND password = '" . md5(md5($_POST['password']).$ec_salt) . "'";
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    //将云起的ID 禁用掉
    /* 获得管理员用户名 */
    $admin_user_info = $db->getRow('SELECT user_name FROM '.$ecs->table('admin_user')." WHERE user_id='$id'");
