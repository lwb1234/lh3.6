...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    if(!empty($ec_salt))
    {
         /* 检查密码是否正确 wang 商家入驻 增加搜索字段,seller_id*/
    ...
    if ($row)
    {
        ...
        // wang 商家入驻 检查是否为入驻商的管理员 所属入驻商家是否有效
        if ($row['seller_id']>0)
        {
            $seller_is_check = seller_info(' is_check = 1 AND id = ' . $row['seller_id']);
            if(empty($seller_is_check))
            {
                sys_msg('对不起，未获得允许，请勿擅闯禁地', 1);
            }
        }
		else
		{
		...
		$_SESSION['seller_id'] = !empty($row['seller_id'])?$row['seller_id']:0;
...
//wang 商家入驻
//获取入驻商信息
function seller_info($conditions = '')
{
    $where = '';
    if (!empty($conditions))
    {
    ...
    /* 查询 */
    $sql = "SELECT id FROM " . $GLOBALS['ecs']->table("user_seller") . " $where";
