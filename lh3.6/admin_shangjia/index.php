...
elseif($_REQUEST['act'] == 'shop_top')
{
	...
	$sql="select id,shop_header,shop_color from ".$ecs->table('seller_shopinfo')." where seller_id='".$_SESSION['seller_id']."'";
	$seller_shop_info=$db->getRow($sql);
	if($seller_shop_info['id']>0)
	{
		/* 创建 百度编辑器 wang 商家入驻 */
    	create_ueditor_editor('shop_header',$seller_shop_info['shop_header']);
...
elseif($_REQUEST['act'] == 'shop_top_edit')
{
	...
	$sql="update ".$ecs->table('seller_shopinfo')." set shop_header='$shop_header',shop_color='$shop_color' where seller_id='".$_SESSION['seller_id']."'";
