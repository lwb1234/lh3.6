...
require_once(ROOT_PATH . '/' . SJ_ADMIN_PATH . '/includes/lib_goods.php');
...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    $smarty->assign('seller_cat_list',     seller_cat_list(0, $cat_id));
	...
	//wang 商家入驻
    $goods_list = goods_list($_REQUEST['act'] == 'list' ? 0 : 1, ($_REQUEST['act'] == 'list') ? (($code == '') ? 1 : 0) : -1," and seller_id='".$_SESSION['seller_id']."'");
...
// wang 商家入驻，获取指定的子分类下拉列表
elseif ($_REQUEST['act'] == 'child_cat')
{
	$first_id=!empty($_REQUEST['first_id'])?intval($_REQUEST['first_id']):0;
	$type=!empty($_REQUEST['type'])?intval($_REQUEST['type']):0;
	$catobj=!empty($_REQUEST['catobj'])?$_REQUEST['catobj']:'';
	$category=child_cat_list($type,$first_id);
	if($category)
	{
		$error=0;
	}
	...
	make_json_response($category,$error,$catobj);
}
/*------------------------------------------------------ */
//-- 添加新商品 编辑商品
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    /* 创建 html editor */
	$smarty->assign('FCKeditor', create_ueditor_editor('goods_desc', $goods['goods_desc']));
	$smarty->assign('FCKeditor2', create_ueditor_editor('packing_list', $goods['packing_list']));
	$smarty->assign('FCKeditor3', create_ueditor_editor('customer_service', $goods['customer_service']));
	...
	$smarty->assign('seller_cat_list',seller_cat_list(0, $goods['seller_cat_id']));
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	//wang 商家入驻
    $catgory_id = empty($_POST['cat_id']) ? (empty($_POST['second_cat_id']) ? (empty($_POST['first_cat_id']) ? '' : intval($_POST['first_cat_id'])) : intval($_POST['second_cat_id'])) : intval($_POST['cat_id']);
	 $seller_cat_id = empty($_POST['seller_cat_id']) ? (empty($_POST['second_cat_id1']) ? (empty($_POST['first_cat_id1']) ? '' : intval($_POST['first_cat_id1'])) : intval($_POST['second_cat_id1'])) : intval($_POST['seller_cat_id']);
    ...
    if ($is_insert)
    {
        ...
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id,seller_cat_id, brand_id, shop_price, market_price, is_promote, promote_price, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id','$seller_cat_id', " .
        ...
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc,add_time, last_update, goods_type, extension_code, rank_integral, suppliers_id,seller_id)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id','$seller_cat_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]','" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$suppliers_id','".$_SESSION['seller_id']."')";
        }
    }
    else
    {
		...
		if($seller_cat_id)
		{
			$sql.="seller_cat_id='$seller_cat_id',";
		}
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
				...
				"check_status=0,".
				"is_display=0 ".
                "WHERE goods_id = '$_REQUEST[goods_id]' and seller_id='".$_SESSION['seller_id']."' LIMIT 1";
    }
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $goods_list = goods_list($is_delete, ($code=='') ? 1 : 0," and seller_id='".$_SESSION['seller_id']."'");
...
//function first_cat_list()
//{
//		$cat_list=array();
//		//网站分类
//        $sql = "select cat_id,cat_name from ".$GLOBALS['ecs']->table('category')." where parent_id=0";
//        
//		$cat_list[0] = $GLOBALS['db']->getAll($sql);
//		
//		//入驻商家分类
//		$sql = "select seller_cat_id as cat_id,cat_name from ".$GLOBALS['ecs']->table('seller_category')." where parent_id=0 and seller_id='".$_SESSION['seller_id']."'";
//        
//		$cat_list[1] = $GLOBALS['db']->getAll($sql);
//		
//		return $cat_list;
//}
//
////wang 商家入驻 获取指定子分类
///*
//* $type 分类对象，0网站分类，1入驻商家分类
//*/
//function child_cat_list($type='0',$parent_id=0)
//{
//	$result=array();
//	if($parent_id>0)
//	{
...
//		else
//		{
//			$sql="select seller_cat_id as cat_id,cat_name,parent_id from ".$GLOBALS['ecs']->table('seller_category')." where parent_id='$parent_id' and seller_id='".$_SESSION['seller_id']."'";
//		
//			$result=$GLOBALS['db']->getAll($sql);
//		}
//	}
