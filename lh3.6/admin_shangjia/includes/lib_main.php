...
function create_kindeditor_editor($input_name, $input_value = '')
{
    global $smarty;
...
function get_goods_list($filter)
{
	...
	//wang 商家入驻
	$where .=" and g.seller_id='".$_SESSION['seller_id']."'";
...
{
    $sql = 'SELECT cat_id, cat_name FROM ' . $GLOBALS['ecs']->table('goods_type') . " WHERE enabled = 1 and seller_id='$_SESSION[seller_id]'";//wang 商家入驻
    $res = $GLOBALS['db']->query($sql);
