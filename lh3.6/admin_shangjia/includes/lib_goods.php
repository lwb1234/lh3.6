...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values,v.attr_value, v.attr_price ".
            ...
            "WHERE a.cat_id = " . intval($cat_id) ." OR a.cat_id = 0 and a.seller_id='$_SESSION[seller_id]'".
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
		...
		//wang 商家入驻
		$filter['seller_cat_id']           = empty($_REQUEST['seller_cat_id']) ? 0 : intval($_REQUEST['seller_cat_id']);
        ...
        //wang 商家入驻
		$filter['status']           = isset( $_REQUEST['status'])?intval($_REQUEST['status']):3;
		...
		//商家入驻 wang
		$where .= $filter['seller_cat_id'] > 0 ? " AND " . get_seller_children($filter['seller_cat_id']) : '';
