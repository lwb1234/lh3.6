...
/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile','order_art_add','order_art_edit','order_art_list','order_art_update','order_art_list2', 'order_list', 'order_detail', 'address_list', 'collection_list',
...
if ($action == 'register')
{
    ...
    $smarty->assign('back_act', $back_act);//888
...
//888
elseif ($action == 'order_art_list2')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');

    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;

    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE user_id = '$user_id' AND order_status = 5 AND shipping_status = 2 AND pay_status = 2");

    $pager  = get_pager('user.php', array('act' => $action), $record_count, $page);

    $orders = get_user_order_arts($user_id, $pager['size'], $pager['start']);

    $smarty->assign('pager',  $pager);
    $smarty->assign('orders', $orders);
    $smarty->display('user_transaction.dwt');
}
elseif ($action == 'order_art_list')
{
    $user_name = $_SESSION['user_name'];
    $where = ' cat_id = 12 AND author = "'.$user_name.'"';
    $smarty->assign('artciles_list',    get_user_articles($where, 1, 100));
	$smarty->assign('user_id',      $user_id);
	$smarty->display('user_transaction.dwt');
}

elseif ($action == 'order_art_edit')
{
    $cur_user_id = isset($_GET['user_id']) ? intval($_GET['user_id']) : 0;
	
	if ($cur_user_id != $user_id)
	{
		show_message('该晒单不属于当前登录用户!');
	}
...
elseif ($action == 'order_art_update')
{
    ...
    if ($exc->edit("title='$_POST[title]', cat_id='$_POST[article_cat]', article_type='$_POST[article_type]', is_open='$_POST[is_open]', author='$_POST[author]', file_url ='$file_url', open_type='$open_type', content='$_POST[FCKeditor1]', order_id='$_POST[order_id]'", $_POST['id']))
    {

		
        show_message('晒单修改成功!', '返回我的晒单', 'user.php?act=order_art_list');
...
elseif ($action == 'order_art_insert')
{
    require_once(ROOT_PATH . 'includes/cls_image.php');
	require_once(ROOT_PATH . 'admin/includes/cls_exchange.php');
	/*初始化数据交换对象 */
    ...
    $sql = "INSERT INTO ".$ecs->table('article')."(title, cat_id, article_type, is_open, author, ".
                "author_email, keywords, content, add_time, file_url, open_type, link, description, order_id) ".
            "VALUES ('$_POST[title]', '$_POST[article_cat]', '$_POST[article_type]', '$_POST[is_open]', ".
                "'$_POST[author]', '', '', '$_POST[FCKeditor1]', ".
                "'$add_time', '$file_url', '$open_type', '', '', '$_POST[order_id]')";
    $db->query($sql);
	
	$new_article_id = $db->insert_id();
	
	
	if (!empty($_POST['order_goods_list']))
	{
		$order_goods_id = explode('|', $_POST['order_goods_list']);
		foreach ($order_goods_id as $goods_id)
		{
			$user_name = $_SESSION['user_name'];
			$sql = "INSERT INTO " .$GLOBALS['ecs']->table('comment') .
				   "(comment_type, id_value, email, user_name, content, comment_rank, add_time, ip_address, status, parent_id, user_id, art_id, order_id) VALUES " .
				   "('0', '" .$goods_id. "', '', '$user_name', '" .$_POST["goods_comment"]."', '5', ".gmtime().", '".real_ip()."', '0', '0', '$user_id', '$new_article_id', '$_POST[order_id]' )";
	...
	show_message('晒单发布成功,请等管理员审核!', '返回我的晒单', 'user.php?act=order_art_list');
...
elseif ($action == 'act_add_booking')
{
    ...
    $booking = array(
		...
		'seller_id' =>isset($_POST['s_id'])  ? intval($_POST['s_id']) : 0    //20150718add
    );
...
function get_user_articles($where, $page = 1, $size = 20)
{
    ...
    $sql = 'SELECT article_id,order_id,is_open, title, author, click, add_time, file_url, open_type, keywords, description' .
...
//888
function get_user_order_arts($user_id, $num = 10, $start = 0)
{
    /* 取得订单列表 */
    $arr    = array();
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time, " .
           ...
           " WHERE user_id = '$user_id' AND order_status = 5 AND shipping_status = 2 AND pay_status = 2 ORDER BY add_time DESC";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$record_count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('article'). " WHERE author = '".$_SESSION['user_name']."' AND cat_id = 12 AND order_id = ".$row['order_id']);
        ...
        $arr[] = array('order_id'       => $row['order_id'],
					   ...
					   'is_comment_order'      => $is_comment_order,
                       'handler'        => $row['handler']);
    }
