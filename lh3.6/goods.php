...
// var_dump($goods);exit;
    ...
    else
    {
		...
		/* add by wengwenjin 商品详细页仿淘宝运费查询插件 begin */
		...
		$smarty->assign('shop_province', $shop_province);
		$smarty->assign('shop_city',     $shop_city);
		$shippings = array();
		$res = $db->GetAll("SELECT shipping_name, shipping_id FROM " . $ecs->table('shipping') . "");
		foreach ($res as $value)
		{
			$areas = array();
			$res1 = $db->GetAll("SELECT * FROM " . $ecs->table('shipping_area') . " WHERE shipping_id = $value[shipping_id]");
		...
		$res = $db->GetAll("SELECT region_id, region_name FROM " . $ecs->table('region') . " WHERE parent_id = 1");
		...
		/* add by wengwenjin 商品详细页仿淘宝运费查询插件 end */

        if ($goods['brand_id'] > 0)
        {
            $goods['goods_brand_url'] = build_uri('brand', array('bid'=>$goods['brand_id']), $goods['goods_brand']);
			/////20150721add 取logo////////////////////////////////////////////////////////////////////////
			$sql = 'SELECT brand_logo FROM ' . $GLOBALS['ecs']->table('brand') ." WHERE brand_id = " . $goods['brand_id'];
			$row = $GLOBALS['db']->getRow($sql);
			$goods['brand_logo']=$row['brand_logo'];
			/////////////////////////////////////////////////////////////////////////////////////////////
        }
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//20150718add wang 商家入驻获取商品的店家信息
		if($goods)
		{
